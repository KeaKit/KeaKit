...
const MyArticlesScreen: React.FC = () => {
  ...
  const [articleToReturn, setArticleToReturn] = useState<UserArticle | null>(
    null,
  );
  const [isProcessingReturn, setIsProcessingReturn] = useState(false);

  const hydrateRentedArticles = useCallback(
    async (data: UserArticle[]) => {
      if (!user) return data;

      const rentedArticles = data.filter(
        (article) => article.status === "RENTED",
      );
      if (rentedArticles.length === 0) return data;

      const rentalEntries: Array<readonly [number, ArticleRecordDTO[]]> =
        await Promise.all(
  ...
  const handleProcessReturn = async (condition: "GOOD" | "DAMAGED") => {
    ...
    try {
      const response = await processArticleReturn(
        articleToReturn.id,
        user.id,
        condition,
        user.token,
  ...
  const getFilterLabel = (f: FilterType): string => {
    switch (f) {
      case "ALL":
        return `Todos (${articles.length})`;
      case "AVAILABLE":
        return `Disponibles (${articles.filter((a) => a.status === "AVAILABLE").length})`;
      case "RENTED":
        return `Alquilados (${articles.filter((a) => a.status === "RENTED").length})`;
    }
  };
    ...
    if (!item.rentals || item.rentals.length === 0) {
      return (
        <Text style={styles.noRentalsText}>
          Sin historial de alquileres todavía.
        </Text>
      );
    }
    ...
    return (
      <View style={styles.rentalsWrapper}>
          ...
          <View key={index} style={styles.rentalItem}>
            <View style={styles.rentalMainInfo}>
              <Text
                style={{ color: "#007AFF" }}
                onPress={() =>
                  navigateToUserReviews(rental.tenantId, rental.tenantName)
                }
              >
                {rental.tenantName}
              </Text>
              <Text
                style={styles.rentalCity}
              >{`${rental.city ?? ""}, ${rental.country ?? ""}`}</Text>
            </View>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate("ArticleRentals", {
              articleId: item.id,
        ...
        >
          <Text style={styles.viewMoreText}>
            Ver historial completo ({item.rentals.length})
          </Text>
          ...
          >
            ...
            <View style={styles.articleInfo}>
              ...
              <View style={styles.priceRow}>
                ...
                >{`€${item.pricePerMonth.toFixed(2)}/mes`}</Text>
              </View>
              <View
                style={[
                  styles.statusBadge,
          ...
          <View style={styles.headerRight}>
            ...
            >
              <Ionicons
                ...
                size={24}
                color={activeAdvancedFiltersCount > 0 ? Colors.primary : "#666"}
              />
        ...
        {filteredArticles.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              Pulsa el botón + para subir tu primer artículo
            </Text>
          </View>
        ) : (
          <FlatList
              ...
              <View style={styles.priceRangeContainer}>
                <View
                  style={[
                    ...
                    priceError ? styles.priceInputWrapperError : null,
                  ]}
                >
                  <TextInput
          ...
          <View style={styles.returnModalOverlay}>
            <View style={styles.returnModalCard}>
              ...
              <Text style={styles.returnModalSubtitle}>
                ¿En qué estado ha devuelto el arrendatario el artículo{" "}
                <Text style={{ fontWeight: "bold", color: "#333" }}>
                  "{articleToReturn?.title}"
                </Text>
                ?
              </Text>
              <Text style={styles.returnModalHint}>
                La evaluación solo está disponible el día de la fecha de fin del
                alquiler.
              </Text>
                ...
                <View style={styles.returnModalActions}>
                  ...
                  >
                    ...
                    <View style={styles.actionCardText}>
                      ...
                      <Text style={styles.actionCardDesc}>
                        Se devolverá la garantía al inquilino
                      </Text>
                    </View>
                  </TouchableOpacity>
                  ...
                  >
                    ...
                    <View style={styles.actionCardText}>
                      ...
                      <Text style={styles.actionCardDesc}>
                        Se retendrá la garantía a tu favor
                      </Text>
                    </View>
                  </TouchableOpacity>
                </View>
